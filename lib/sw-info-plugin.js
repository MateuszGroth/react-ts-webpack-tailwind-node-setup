...
module.exports = class ServiceWorkerInfoPlugin {
    constructor({ test = file => file === 'sw.js', filterAssets = () => true } = {}) {
        this.test = test;
    ...
    apply(compiler) {
        compiler.hooks.thisCompilation.tap('ServiceWorkerInfoPlugin', compilation => {
            compilation.hooks.processAssets.tap(
                ...
                assetsList => {
                    ...
                    const referencedAssets = assets
                        .filter(asset => this.filterAssets(asset.name) && !this.test(asset.name))
                        .sort();
                    ...
                    const publicPath = outputOptions.publicPath === 'auto' ? '/' : outputOptions.publicPath || '/';
